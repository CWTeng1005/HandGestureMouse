...
def _ev(n):
    ...
    if isinstance(n, ast.BinOp) and type(n.op) in _BIN: return _BIN[type(n.op)](_ev(n.left), _ev(n.right))
    if isinstance(n, ast.UnaryOp) and type(n.op) in _UN: return _UN[type(n.op)](_ev(n.operand))
...
class GestureCalculator:
    def __init__(self, master=None, topmost=True):
        ...
        try: self.root.attributes("-topmost", bool(topmost))
        except: pass
    ...
    def _build(self):
        ...
        for i in range(1,6): f.rowconfigure(i, weight=1)
        for j in range(4):   f.columnconfigure(j, weight=1)
    ...
    def _a(self, ch): self.var.set(self.var.get()+ch)
    def append_digit(self, d:int): self._a(str(max(0,min(9,int(d)))))
