...
class handDetector():
    ...
    def findPositionBox(self, img, handNo=0, draw=True):
        xList = []
        yList = []
        bbox = []
        self.lmList = []
        ...
        if (len(xList) != 0) and (len(yList) != 0):
            ...
            if draw:
                cv2.rectangle(img, (xmin - 20, ymin - 20), (xmax + 20, ymax + 20), (150, 150, 0), 2)
    ...
    def fingersUp(self):
        ...
        if self.lmList[self.tipIds[0]][1] < self.lmList[self.tipIds[0] - 1][1]:
            # thumb is up (to the left in mirrored image)
            fingers.append(1)
        else:
            fingers.append(0)
    ...
    def findDistance(self, p1, p2, img, draw=True, r=12, t=3):
        ...
        if draw:
            ...
            cv2.circle(img, (cx, cy), r, (150, 150, 0), cv2.FILLED)
